...
})({"56lpG":[function(require,module,exports,__globalThis) {
...
class Task {
    ...
    constructor(title, description, assignedUserId){
        this.id = Task.nextId++;
        this.title = title;
...
function renderTasks() {
    ...
    if (filterId !== null) tasks = tasks.filter((t)=>t.assignedUserId === filterId);
    tasks.forEach((task)=>{
        const user = manager.getUser(task.assignedUserId);
        const li = document.createElement("li");
        li.innerHTML = `<strong>${task.title}</strong> (${task.description}) \u{2192} ${user?.name}<br/>Status: ${task.completed ? "Done" : "Pending"}`;
...
function renderPendingTasks() {
    ...
    tasks.forEach((task)=>{
        const user = manager.getUser(task.assignedUserId);
        const li = document.createElement("li");
        li.textContent = `${task.title} \u{2192} ${user?.name}`;
        list.appendChild(li);
    });
}
renderUserSelect();
...
},{}]},["56lpG","ly4KV"], "ly4KV", "parcelRequire0112", {})

//# sourceMappingURL=Task-ManagerGui.5e0e6471.js.map
